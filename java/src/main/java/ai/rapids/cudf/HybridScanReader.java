 ...
 *   <li>Build a row mask describing which rows survive page-level pruning.</li>
 *   <li>Materialize the <em>filter</em> columns first, mutating the row mask down to only the
 *       rows that survive the AST filter.</li>
 *   <li>Materialize the <em>payload</em> columns using the surviving row mask.</li>
 * </ol>
 *
 * <p>Convenience methods are provided for the common single-call cases
 * ({@link #materializeFromBuffer(HostMemoryBuffer)} and
 * {@link #materializeAllColumns(int[], DeviceMemoryBuffer[])}), but the explicit two-step
 * pipeline is the most efficient when the filter is highly selective.
 *
